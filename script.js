...
// function submitRequirement() {
//     const name = document.getElementById("reqName").value;
//     // const equation = document.getElementById("reqEquation").value;
//     const latexEq = document.getElementById("reqEquation").value;
//     const equation = convertLatexToReadable(latexEq);
...
//     currentRequirement = {
//           name,
//           equation: convertLatexToReadable(latexEq),
//           latexEquation: latexEq,  
//           parameters: []
//         };
...
//     if (seen.has(param.name)) return; // avoid duplicating the same parameter
...
//         param.name,
//         param.type,
//         param.type === "HYBRID" ? param.latexEquation : "", // ✅ FIXED: preserve raw LaTeX
...
//             details.innerHTML = `<p><strong>Hybrid Equation:</strong> \\(${param.latexEquation || param.equation || 'N/A'}\\)</p>`;
...
//         title.innerHTML = `Requirement: \\(${req.name}\\)`;  // ✅ Render as equation
...
//     MathJax.typeset();  // ✅ Render all LaTeX
// }
...
//     const grouped = {};
//     data.forEach(row => {
//         const [reqName, equation, paramName, type, paramEq, admMin, admMax, desMin, desMax, parentChain] = row;
//         if (!grouped[reqName]) grouped[reqName] = { equation, parameters: [] };
//         grouped[reqName].parameters.push({
//             paramName,
//             type,
//             paramEq,
...
//     Object.entries(grouped).forEach(([reqName, { equation, parameters }], reqIndex) => {
//         const section = document.createElement("div");
//         section.className = "requirement-section";
//         section.innerHTML = `
//             <h3>Requirement: 
...
//                         <tr id="paramRow-${reqIndex}-${i}">
//                             <td>
//                                 <math-field virtual-keyboard-mode="onfocus" class="math-field-wide">
//                                     ${p.paramName}
//                                 </math-field>
//                             </td>
//                             <td>
...
//     `;
//     table.appendChild(newRow);
//     MathJax.typeset();  // Render math if needed
// }
...
//             // Get all math-fields in this row in order
//             const mathFields = row.querySelectorAll("math-field");
//             const inputs = row.querySelectorAll("input");
//             const selects = row.querySelectorAll("select");

//             // Ensure we map just like requirement: full LaTeX value
//             const paramName = mathFields[0]?.getValue?.() || "";
//             const type = selects[0]?.value || "";
//             const paramEq = mathFields[1]?.getValue?.() || ""; // ✅ HYBRID equation saved as LaTeX
//             const admMin = inputs[0]?.value || "";
//             const admMax = inputs[1]?.value || "";
//             const desMin = inputs[2]?.value || "";
//             const desMax = inputs[3]?.value || "";
//             const parent = mathFields[2]?.getValue?.() || "";

//             rows.push([reqName, reqEq, paramName, type, paramEq, admMin, admMax, desMin, desMax, parent]);
//         });
//     });
...
//             const normalizedKey = readableReq.replace(/\s+/g, '');
//             const opt = window.optimizationResults?.[normalizedKey];
            
//             if (opt) {
//                 const optVal = opt.value?.toFixed(7);
...
//         // Desirable range label
//         let rawLabel = "N/A";
//         if (desMin?.trim() !== "" && desMax?.trim() !== "") {
//             const dMin = parseFloat(desMin);
//             const dMax = parseFloat(desMax);
//             if (!isNaN(dMin) && !isNaN(dMax)) {
//                 rawLabel = `[${dMin.toFixed(7)} – ${dMax.toFixed(7)}]`;
//             }
//         } else if (desMin?.trim() !== "") {
//             const dMin = parseFloat(desMin);
//             if (!isNaN(dMin)) rawLabel = `≥ ${dMin.toFixed(7)}`;
//         } else if (desMax?.trim() !== "") {
//             const dMax = parseFloat(desMax);
//             if (!isNaN(dMax)) rawLabel = `≤ ${dMax.toFixed(7)}`;
//         }

//         const edgeLabel = `Desirable ∈ ${rawLabel}`;
...
//             edgeLabel = `[${min.toFixed(7)} – ${max.toFixed(7)}]`;
//         } else if (!isNaN(min)) {
//             edgeLabel = `≥ ${min.toFixed(7)}`;
//         } else if (!isNaN(max)) {
//             edgeLabel = `≤ ${max.toFixed(7)}`;
//         }

//         const parent = parentChain || reqName;
//         edges.push({ from: parent, to: paramName, label: `Desirable ∈ ${edgeLabel}` });
//     });
...
//         const reqDiv = document.createElement("div");
//         reqDiv.style.marginTop = "2rem";
//         reqDiv.innerHTML = `<h3>Configure Inputs for: ${reqName}</h3>`;

//         const form = document.createElement("form");
//         form.setAttribute("data-requirement", reqName);

//         reqRows.forEach(row => {
//             const [_, __, paramName, type, , admMin, admMax, desMin, desMax] = row;
//             if (!paramName || type === "HYBRID") return;
...
//     const eqRaw = reqRows[0][1];
//     const mainEq = convertLatexToMathJS(eqRaw);  // ✅ updated to convert LaTeX (e.g., \min) to math.js-compatible

//     const hybridParams = {};
//     reqRows.forEach(row => {
//         const [, , rawParamName, type, paramEq] = row;
//         const paramName = normalize(rawParamName);
//         if (type === "HYBRID" && paramEq?.trim()) {
//             hybridParams[paramName] = {
//                 expr: convertLatexToMathJS(paramEq)  // ✅ also parse hybrid parameter equations
//             };
//         }
//     });
...
//     combinations.forEach(combo => {
//         const scope = {};
//         paramNames.forEach((name, i) => scope[name] = combo[i]);
